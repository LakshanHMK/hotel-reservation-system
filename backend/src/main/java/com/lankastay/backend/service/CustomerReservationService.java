...
public class CustomerReservationService {
    ...
    public CustomerReservationService(ReservationRepository reservations, ReservationItemRepository items,
                                      ...
                                      OfferRepository offers, SecurityAuditService audit) {
    ...
    public ReservationResponse create(UUID customerId, CreateReservationRequest request) {
        ...
        BigDecimal averageNightly = subtotal.divide(BigDecimal.valueOf(nights * request.quantity()), 2, RoundingMode.HALF_UP);
        ...
        if (request.offerId() != null) {
            ...
            EvaluatedOffer evaluated = evaluateOffer(candidate, subtotal, nights, request.quantity(), hotel.getId(), room,
                    request.checkIn(), request.checkOut(), LocalDate.now());
            if (evaluated == null) {
                throw new ConflictException("The requested offer is no longer valid or eligible for your stay. Please review the updated quote.");
            }
    ...
    @Transactional(readOnly = true)
    public AvailabilityResponse availability(Long hotelId, Long roomId, LocalDate checkIn, LocalDate checkOut, int quantity) {
        if (hotelId == null || roomId == null || checkIn == null || checkOut == null || !checkIn.isBefore(checkOut) || quantity < 1) {
        ...
        for (LocalDate night = checkIn; night.isBefore(checkOut); night = night.plusDays(1)) {
            int available = room.getInventoryCount() - Math.toIntExact(items.bookedQuantityForNight(roomId, night));
            minimum = Math.min(minimum, available);
            if (available < quantity && unavailable == null) unavailable = night;
        }
    ...
    public ReservationQuoteResponse quote(ReservationQuoteRequest request) {
        ...
        Hotel hotel = hotels.findById(request.hotelId()).orElseThrow(() -> badRequest("The selected hotel does not exist."));
        if (!HotelStatus.PUBLICATION_ACTIVE.equalsIgnoreCase(hotel.getStatus()) || !HotelStatus.PUBLICATION_ACTIVE.equalsIgnoreCase(hotel.getPublicationStatus())) {
            throw badRequest("The selected hotel is not available for booking.");
        }
        Room room = rooms.findById(request.roomId()).orElseThrow(() -> badRequest("The selected room does not exist."));
        CreateReservationRequest validation = new CreateReservationRequest(request.hotelId(), request.roomId(), request.rateId(),
        ...
        RoomRate rate = rates.findById(request.rateId()).orElseThrow(() -> badRequest("The selected rate does not exist."));
        validateRate(validation, rate);
        AvailabilityResponse availability = availability(request.hotelId(), request.roomId(), request.checkIn(), request.checkOut(), request.quantity());
        ...
        for (LocalDate date = request.checkIn(); date.isBefore(request.checkOut()); date = date.plusDays(1)) {
            ...
            BigDecimal amount = weekend && rate.getWeekendNightlyRate() != null && rate.getWeekendNightlyRate().compareTo(BigDecimal.ZERO) > 0
        ...
        return new ReservationQuoteResponse(availability.available(), availability.availableQuantity(), nightly.size(), nightly,
    ...
    private void validateDatesAndGuests(CreateReservationRequest request) {
        if (!request.checkIn().isBefore(request.checkOut())) throw badRequest("Check-out must be after check-in.");
        if (request.checkIn().isBefore(LocalDate.now())) throw badRequest("Check-in cannot be in the past.");
    }

    private void validateRoom(CreateReservationRequest request, Room room) {
        if (!Objects.equals(room.getHotelId(), request.hotelId())) throw badRequest("The selected room does not belong to this hotel.");
        if (!"ACTIVE".equalsIgnoreCase(room.getStatus()) || room.getInventoryCount() == null || room.getInventoryCount() < 1) {
            throw badRequest("The selected room is not available for booking.");
        }
        if (request.quantity() > room.getInventoryCount()) throw new ConflictException(AVAILABILITY_MESSAGE);
    ...
    private void validateRate(CreateReservationRequest request, RoomRate rate) {
        if (!Objects.equals(rate.getHotelId(), request.hotelId()) || !Objects.equals(rate.getRoomId(), request.roomId())) {
            throw badRequest("The selected rate does not belong to this hotel and room.");
        }
        if (!"ACTIVE".equalsIgnoreCase(rate.getStatus()) || rate.getBaseNightlyRate() == null || rate.getBaseNightlyRate().compareTo(BigDecimal.ZERO) <= 0) {
    ...
    private void assertAvailable(Room room, LocalDate checkIn, LocalDate checkOut, int requested) {
        for (LocalDate night = checkIn; night.isBefore(checkOut); night = night.plusDays(1)) {
            ...
            if (room.getInventoryCount() - booked < requested) throw new ConflictException(AVAILABILITY_MESSAGE);
        }
    }
    ...
    private EvaluatedOffer evaluateOffer(Offer offer, BigDecimal subtotal, long nights, int quantity,
        ...
        // Hotel target check
        if (!offer.getTargetHotelIds().isEmpty() && !offer.getTargetHotelIds().contains(hotelId)) {
            return null;
        }

        // Room target check
        if (!offer.getTargetRoomTypeIds().isEmpty() && !offer.getTargetRoomTypeIds().contains(room.getId())) {
            return null;
        }
    ...
    private EvaluatedOffer findBestOffer(BigDecimal subtotal, long nights, int quantity,
        ...
        for (Offer offer : activeOffers) {
            EvaluatedOffer evaluated = evaluateOffer(offer, subtotal, nights, quantity, hotelId, room, checkIn, checkOut, bookingDate);
            if (evaluated != null && evaluated.discountAmount().compareTo(BigDecimal.ZERO) > 0) {
                if (best == null || evaluated.discountAmount().compareTo(best.discountAmount()) > 0
                        || (evaluated.discountAmount().compareTo(best.discountAmount()) == 0 && evaluated.offer().getId() < best.offer().getId())) {
                    best = evaluated;
                }
            }
        }
    ...
    private String generateReference() {
        for (int attempt = 0; attempt < 20; attempt++) {
            String candidate = "LS-" + LocalDate.now().getYear() + "-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase(Locale.ROOT);
            if (!reservations.existsByReservationCode(candidate)) return candidate;
        }
    ...
    private List<ReservationResponse> map(List<Reservation> found) {
        ...
        items.findByReservationIdIn(found.stream().map(Reservation::getId).toList()).forEach(item ->
                byReservation.computeIfAbsent(item.getReservationId(), ignored -> new ArrayList<>()).add(ReservationItemResponse.from(item)));
        return found.stream().map(r -> ReservationResponse.from(r, byReservation.getOrDefault(r.getId(), List.of()))).toList();
    }
    ...
    private ApiException badRequest(String message) { return new ApiException(HttpStatus.BAD_REQUEST, "Bad Request", message); }
