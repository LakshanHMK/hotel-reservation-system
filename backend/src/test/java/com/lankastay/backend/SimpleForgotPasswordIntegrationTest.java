...
class SimpleForgotPasswordIntegrationTest {
    ...
    private static final String EMAIL = "customer.forgot@lankastay.test";
    ...
    @BeforeEach void setup() {
        tokens.deleteAll();
        customers.deleteAll();
        CustomerUser account = new CustomerUser();
    ...
    @Test void knownAndUnknownEmailsHaveIdenticalGenericResponse() throws Exception {
        String known = mvc.perform(post("/api/v1/customer/auth/forgot-password").with(csrf())
        ...
        String unknown = mvc.perform(post("/api/v1/customer/auth/forgot-password").with(csrf())
    ...
    @Test void mismatchLeavesPasswordUnchangedThenValidTokenReplacesHashAndLoginCredential() throws Exception {
        String before = customers.findByEmail(EMAIL).orElseThrow().getPasswordHash();
        mvc.perform(post("/api/v1/customer/auth/forgot-password").with(csrf()).contentType("application/json")
        ...
        mvc.perform(post("/api/v1/customer/auth/reset-password").with(csrf()).contentType("application/json")
                .content("{\"token\":\"" + raw + "\",\"newPassword\":\"" + NEW_PASSWORD + "\",\"confirmNewPassword\":\"Different3#\"}"))
                .andExpect(status().isBadRequest());
        assertThat(customers.findByEmail(EMAIL).orElseThrow().getPasswordHash()).isEqualTo(before);
        mvc.perform(post("/api/v1/customer/auth/reset-password").with(csrf()).contentType("application/json")
                .content("{\"token\":\"" + raw + "\",\"newPassword\":\"" + NEW_PASSWORD + "\",\"confirmNewPassword\":\"" + NEW_PASSWORD + "\"}"))
                .andExpect(status().isOk());
        String saved = customers.findByEmail(EMAIL).orElseThrow().getPasswordHash();
