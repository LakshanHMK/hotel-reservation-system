...
function MyReservations() {
  ...
  const { getReviewForReservation, canCustomerReviewReservation } = useReviews();
  ...
  const cancellationHotel = cancellationReservation
    ? hotels.find((hotel) => String(hotel.id) === String(cancellationReservation.hotelId))
    : null;
  const cancellationRoom = cancellationReservation
    ? rooms.find((room) => String(room.id) === String(cancellationReservation.roomId))
    : null;
          ...
          <div className="reservation-list">
            {visibleReservations.map((reservation) => {
              const hotel = hotels.find(
                (item) => String(item.id) === String(reservation.hotelId),
              );
              const room = rooms.find(
                (item) => String(item.id) === String(reservation.roomId),
              );
