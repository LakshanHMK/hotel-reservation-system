...
export default function ReviewEditor() {
  ...
  if ((reservationId && !eligibility?.eligible) || (editing && !allowedEdit) || !reservation || !hotel || !room) {
    return (
      <main className="review-editor-page">
        <section className="review-unavailable">
          <ShieldCheck size={32} />
          ...
          <p>{reason || 'This Review cannot be edited from the current account.'}</p>
          <div>
            {eligibility?.review && (
              <>
                <Link to={`/reviews/${eligibility.review.id}`}>View My Review</Link>
                <Link to={`/reviews/${eligibility.review.id}/edit`}>Edit Review</Link>
              </>
            )}
    ...
    <main className="review-editor-page">
      ...
      <form className="review-form-card" onSubmit={submit} noValidate>
        ...
        <footer>
          ...
          <button type="submit">{editing ? 'Save Review Changes' : 'Submit Review'}</button>
        </footer>
      </form>
      {policy && (
        <ManagementDialog
