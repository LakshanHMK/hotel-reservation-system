...
export function ReviewsProvider({ children }) {
  ...
  const refreshReviews = useCallback(async () => {
    ...
    try {
      ...
      // Public reviews
      const publicData = await reviewApi.listPublicReviews()
    ...
    async (reviewId, _customerId) => {
      try {
        ...
        setReviewStays((prev) => prev.map((stay) => String(stay.reviewId) === String(reviewId)
          ? { ...stay, eligible: false, alreadyReviewed: true, reviewId: null }
          : stay))
        return { success: true }
      } catch (err) {
  ...
  const value = useMemo(
    () => ({
      ...
      canCustomerReviewReservation: (customerId, reservationId) => {
        ...
        if (!stay) return { eligible: false, reason: 'NOT_FOUND' }
